...
public class RsvCounter {
    ...
    @Id
    @Column(name = "counter_date", length = 8)
    private String counterDate;     // 예: 20251007

    @Id
    @Column(name = "target_type", length = 3)
    private String targetType;      // 예: ACC(숙소), FLY(항공), DLV(짐)

    @Column(name = "next_seq", nullable = false)
    private Integer nextSeq;        // 해당 날짜/유형의 다음 시퀀스 번호
